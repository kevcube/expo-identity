import { decodeFirstSync, encode as cborEncode } from "cbor";
import { Buffer } from "buffer";
import { createHash, createPublicKey, createPrivateKey, webcrypto } from "node:crypto";
...
function toBuffer(input: string | Uint8Array | Buffer, encoding: BufferEncoding = "base64") {
...
function toArrayBuffer(input: string | Uint8Array | Buffer, encoding: BufferEncoding = "base64") {
...
export function decodeVerifyIdentityEnvelope(input: string | Uint8Array | Buffer): VerifyIdentityEnvelope {
  const envelope = decodeFirstSync(toBuffer(input)) as VerifyIdentityEnvelope;
...
export function computeEncryptionKeyHashFromCertificate(certPem: string): Buffer {
...
export function buildSessionTranscript(
  ...
  return Buffer.from(
    cborEncode([
      null,
      null,
      [CONTEXT_STRING, Buffer.from(nonce), merchantIdentifier, teamIdentifier, Buffer.from(encryptionKeyHash)],
    ])
  );
}
...
async function importPrivateKey(pem: string): Promise<CryptoKey> {
  ...
  const derBuffer = pkcs8Der.buffer.slice(pkcs8Der.byteOffset, pkcs8Der.byteOffset + pkcs8Der.byteLength);
...
export async function decryptVerifyIdentityResponse(
  ...
  const plaintextArrayBuffer = await recipientContext.open(ciphertext, EMPTY_AAD);
  const plaintext = Buffer.from(new Uint8Array(plaintextArrayBuffer));
  const identity = decodeFirstSync(plaintext);
