...
export type RetentionPolicy =
  | { retain: false }
  | { retain: true; retentionDays: number };
...
export type IdentityRequestDefinitions = Record<string, IdentityRequestDefinition>;

export type JsonPrimitive = string | number | boolean | null;
export type JsonValue = JsonPrimitive | JsonValue[] | { [key: string]: JsonValue };
export type IdentityByteValue = { base64url: string; mediaType?: string };

export type IdentityClaimOutput<TClaim extends IdentityClaim> =
  TClaim extends { type: 'ageAtLeast' }
    ? boolean
...
export function validateIdentityRequests(
  ...
  for (const [requestKey, request] of Object.entries(requests)) {
    ...
    requireNonempty(request.document?.doctype, `requests.${requestKey}.document.doctype`);
    requireNonempty(request.document?.namespace, `requests.${requestKey}.document.namespace`);
    if (
    ...
    for (const [alias, claim] of Object.entries(request.claims)) {
      ...
      if (claim.retain === true) {
        if (!Number.isInteger(claim.retentionDays) || claim.retentionDays <= 0) {
          throw new TypeError(`${path}.retentionDays must be a positive integer`);
        }
      } else if (claim.retain === false) {
        if ('retentionDays' in claim) {
          throw new TypeError(`${path}.retentionDays is allowed only when retain is true`);
        }
      } else {
      ...
      if (claim.type === 'ageAtLeast') {
        if (!Number.isInteger(claim.age) || claim.age < 0 || claim.age > 150) {
          throw new TypeError(`${path}.age must be an integer from 0 through 150`);
        }
